...
lli solve(int pos, int sume, int sumo, int t, int nozero)
{
    ...
    if (pos == k)
    {
        if (sume - sumo > 1 && prime[abs(sume - sumo)])
            return 1;
        else
            return 0;
    }
    ...
    for (int i = 0; i <= lim; i++)
    {
        if (i != lim)
            ...
            nt = t;
        int newzero = (nozero || i > 0);
        if (newzero)
            ans += solve(pos + 1, sume + ((k - pos + 1) % 2 ? i : 0), sumo + ((k - pos + 1) % 2 ? 0 : i), nt, newzero);
        else
            ans += solve(pos + 1, sume, sumo, nt, newzero);
    }
    return dp[pos][sume][sumo][t][nozero] = ans;
}
int main()
{
    kira;
